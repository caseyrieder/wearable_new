...
export function discoverBag(bag: any, bagsState: any) {
    ...
    if (bag.name) { console.log('Named: ', bag); }
    if (!bag.name) { bag.name = 'NO NAME'; }
    bags.set(bag.id, bag);
    return bags;
}
...
export function getDiscoveredBags(bagsState: any) {
    BleManager.getDiscoveredPeripherals([]).then(discs => {
        if (discs.length == 0) { console.log('No discovered peripherals'); }
...
export function getConnectedBags(bagsState: any) {
    BleManager.getConnectedPeripherals([]).then(res => {
        if (res.length == 0) { console.log('No connected bags'); }
        console.log(res);
        var bags = bagsState;
...
export function writeAllCharacteristics(periph: string, obj: BLEArray) {
    setTimeout(() => {
      let sendPin = stringToBytes(obj[0].data);
      let sendMessage = stringToBytes(obj[1].data);
      let sendColor = (typeof obj[2].data === "string" ? hex2Rgb(obj[2].data) : obj[2].data);
      let sendSpeed = [obj[3].data];
      let sendDirection = [obj[4].data];
      let sendBrightness = [obj[5].data];
      BleManager.write(periph, UUIDs.svc, UUIDs.pin, sendPin).then(() => {
        console.log(`Wrote ${obj[0].char}: ${obj[0].data}}'\n`);
        BleManager.write(periph, UUIDs.svc, UUIDs.msg, sendMessage).then(() => {
          console.log(`Wrote ${obj[1].char}: ${obj[1].data}}'\n`);
          BleManager.write(periph, UUIDs.svc, UUIDs.clr, sendColor).then(() => {
              console.log(`Wrote ${obj[2].char}: ${obj[2].data}}'\n`);
              BleManager.write(periph, UUIDs.svc, UUIDs.spd, sendSpeed).then(() => {
                console.log(`Wrote ${obj[3].char}: ${obj[3].data}}'\n`);
                BleManager.write(periph, UUIDs.svc, UUIDs.dir, sendDirection).then(() => {
                  console.log(`Wrote ${obj[4].char}: ${obj[4].data}}'\n`);
                  BleManager.write(periph, UUIDs.svc, UUIDs.svc, sendBrightness).then(() => {

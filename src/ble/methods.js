...
async function pair(bagId, pin) {
    ...
    getServices(bagId)
    await BleManager.write(bagId, svcUuid, UUIDs.pin, stringToBytes(pin)).then(() => {
...
async function getPinAndBagId() {
    ...
    let values;
    try {
        values = await AsyncStorage.multiGet(['@wearable_bag_id', '@wearable_bag_pin'])
    } catch (e) {
...
function writeAll(bagId, dataObj) {
    ...
    setTimeout(() => {
        ...
        BleManager.write(bagId, svcUuid, dataObj[0].uuid, stringToBytes(dataObj[0].data)).then(() => {
            console.log(`Wrote pin: ${pin}\n`);
            BleManager.write(bagId, svcUuid, dataObj[1].uuid, stringToBytes(dataObj[1].data)).then(() => {
                console.log(`Wrote ${dataObj[1].char}: ${dataObj[1].data}\n`);
                BleManager.write(bagId, svcUuid, dataObj[2].uuid, hex2Rgb(dataObj[2].data)).then(() => {
                    console.log(`Wrote ${dataObj[2].char}: ${dataObj[2].data}\n`);
                    BleManager.write(bagId, svcUuid, dataObj[3].uuid, [dataObj[3].data]).then(() => {
                        console.log(`Wrote ${dataObj[3].char}: ${dataObj[3].data}\n`);
                        BleManager.write(bagId, svcUuid, dataObj[4].uuid, [dataObj[4].data]).then(() => {
                            console.log(`Wrote ${dataObj[4].char}: ${dataObj[4].data}\n`);
                            BleManager.write(bagId, svcUuid, dataObj[5].uuid, [dataObj[5].data]).then(() => {

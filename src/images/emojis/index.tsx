import HeartYellow from './heart-yellow.png';
import HeartRed from './heart-red.png';
import HeartWhite from './heart-white.png';
import HeartGreen from './heart-green.png';
import HeartBlue from './heart-blue.png';
import BrokenHeartYellow from './brokenheart-yellow.png';
import BrokenHeartRed from './brokenheart-red.png';
import BrokenHeartWhite from './brokenheart-white.png';
import BrokenHeartGreen from './brokenheart-green.png';
import BrokenHeartBlue from './brokenheart-blue.png';
import StarYellow from './star-yellow.png';
import StarRed from './star-red.png';
import StarWhite from './star-white.png';
import StarGreen from './star-green.png';
import StarBlue from './star-blue.png';
import LightningYellow from './lightning-yellow.png';
import LightningRed from './lightning-red.png';
import LightningWhite from './lightning-white.png';
import LightningGreen from './lightning-green.png';
import LightningBlue from './lightning-blue.png';
import UpYellow from './up-yellow.png';
import UpRed from './up-red.png';
import UpWhite from './up-white.png';
import UpGreen from './up-green.png';
import UpBlue from './up-blue.png';
import DownYellow from './down-yellow.png';
import DownRed from './down-red.png';
import DownWhite from './down-white.png';
import DownGreen from './down-green.png';
import DownBlue from './down-blue.png';
import LeftYellow from './left-yellow.png';
import LeftRed from './left-red.png';
import LeftWhite from './left-white.png';
import LeftGreen from './left-green.png';
import LeftBlue from './left-blue.png';
import RightYellow from './right-yellow.png';
import RightRed from './right-red.png';
import RightWhite from './right-white.png';
import RightGreen from './right-green.png';
import RightBlue from './right-blue.png';
import DimplesRed from './face-dimples-red.png';
import StraightEyesRed from './face-straight-eyes-red.png';
import SquintingRed from './face-squint-eyes-red.png';
import BigEyesTriangleNoseRed from './face-big-eyes-triangle-nose-red.png';
import BigEyesLineMouthRed from './face-big-eyes-line-mouth-red.png';
import Fire from './fire.png';

// < \u0020
// > \u007E && < \u009D
export const emojis = [
  {
    index: 1,
    code: '\u0084',
    color: '#E20075',
    charLength: 7,
    ascii: '\u2002',
    image: StraightEyesRed,
  },
  {
    index: 2,
    code: '\u008E',
    color: '#E20075',
    charLength: 7,
    ascii: '\u2000',
    image: BigEyesTriangleNoseRed,
  },
  {
    index: 3,
    code: '\u0089',
    color: '#E20075',
    charLength: 7,
    ascii: '\u2003',
    image: SquintingRed,
  },
  {
    index: 5,
    code: '\u007F',
    color: '#E20075',
    charLength: 5,
    ascii: '\u2004',
    image: BigEyesLineMouthRed,
  },
  {
    index: 4,
    code: '\u0093',
    color: '#E20075',
    charLength: 7,
    ascii: '\u2005',
    image: DimplesRed,
  },
  {
    index: 6,
    code: '\u0097',
    color: '#E20075',
    charLength: 2,
    ascii: '\u2005',
    image: Fire,
  },
  {
    index: 7,
    code: '\u0001',
    color: '#FAD329',
    charLength: 2,
    ascii: '\u3001',
    image: HeartYellow,
  },
  {
    index: 8,
    code: '\u0002',
    color: '#E20075',
    charLength: 2,
    ascii: '\u3002',
    image: HeartRed,
  },
  {
    index: 9,
    code: '\u0003',
    color: '#FFFFFF',
    charLength: 2,
    ascii: '\u3003',
    image: HeartWhite,
  },
  {
    index: 10,
    code: '\u0004',
    color: '#65C8C0',
    charLength: 2,
    ascii: '\u3004',
    image: HeartGreen,
  },
  {
    index: 11,
    code: '\u0005',
    color: '#53BAF2',
    charLength: 2,
    ascii: '\u3005',
    image: HeartBlue,
  },
  {
    index: 12,
    code: '\u0006',
    color: '#FAD329',
    charLength: 3,
    ascii: '\u3006',
    image: BrokenHeartYellow,
  },
  {
    index: 13,
    code: '\u0007',
    color: '#E20075',
    charLength: 3,
    ascii: '\u3007',
    image: BrokenHeartRed,
  },
  {
    index: 14,
    code: '\u0008',
    color: '#FFFFFF',
    charLength: 3,
    ascii: '\u3008',
    image: BrokenHeartWhite,
  },
  {
    index: 15,
    code: '\u0009',
    color: '#65C8C0',
    charLength: 3,
    ascii: '\u3009',
    image: BrokenHeartGreen,
  },
  {
    index: 16,
    code: '\u000A',
    color: '#53BAF2',
    charLength: 3,
    ascii: '\u300A',
    image: BrokenHeartBlue,
  },
  {
    index: 17,
    code: '\u000B',
    color: '#FAD329',
    charLength: 2,
    ascii: '\u3010',
    image: StarYellow,
  },
  {
    index: 18,
    code: '\u0011',
    color: '#E20075',
    charLength: 2,
    ascii: '\u3011',
    image: StarRed,
  },
  {
    index: 19,
    code: '\u0012',
    color: '#FFFFFF',
    charLength: 2,
    ascii: '\u3012',
    image: StarWhite,
  },
  {
    index: 20,
    code: '\u0013',
    color: '#65C8C0',
    charLength: 2,
    ascii: '\u3013',
    image: StarGreen,
  },
  {
    index: 21,
    code: '\u0014',
    color: '#53BAF2',
    charLength: 2,
    ascii: '\u3014',
    image: StarBlue,
  },
  {
    index: 22,
    code: '\u0010',
    color: '#FAD329',
    charLength: 3,
    ascii: '\u300B',
    image: LightningYellow,
  },
  {
    index: 23,
    code: '\u000C',
    color: '#E20075',
    charLength: 3,
    ascii: '\u300C',
    image: LightningRed,
  },
  {
    index: 24,
    code: '\u000D',
    color: '#FFFFFF',
    charLength: 3,
    ascii: '\u300D',
    image: LightningWhite,
  },
  {
    index: 25,
    code: '\u000E',
    color: '#65C8C0',
    charLength: 3,
    ascii: '\u300E',
    image: LightningGreen,
  },
  {
    index: 26,
    code: '\u0014',
    color: '#53BAF2',
    charLength: 3,
    ascii: '\u300F',
    image: LightningBlue,
  },
  {
    index: 27,
    code: '\u000F',
    color: '#FAD329',
    charLength: 3,
    ascii: '\u3015',
    image: UpYellow,
  },
  {
    index: 28,
    code: '\u0016',
    color: '#E20075',
    charLength: 3,
    ascii: '\u3016',
    image: UpRed,
  },
  {
    index: 29,
    code: '\u0017',
    color: '#FFFFFF',
    charLength: 3,
    ascii: '\u3017',
    image: UpWhite,
  },
  {
    index: 30,
    code: '\u0018',
    color: '#65C8C0',
    charLength: 3,
    ascii: '\u3018',
    image: UpGreen,
  },
  {
    index: 31,
    code: '\u0019',
    color: '#53BAF2',
    charLength: 3,
    ascii: '\u3019',
    image: UpBlue,
  },
  {
    index: 32,
    code: '\u001F',
    color: '#FAD329',
    charLength: 3,
    ascii: '\u301F',
    image: RightYellow,
  },
  {
    index: 33,
    code: '\u0020',
    color: '#E20075',
    charLength: 3,
    ascii: '\u3020',
    image: RightRed,
  },
  {
    index: 34,
    code: '\u0021',
    color: '#FFFFFF',
    charLength: 3,
    ascii: '\u3021',
    image: RightWhite,
  },
  {
    index: 35,
    code: '\u0022',
    color: '#65C8C0',
    charLength: 3,
    ascii: '\u3022',
    image: RightGreen,
  },
  {
    index: 36,
    code: '\u0023',
    color: '#53BAF2',
    charLength: 3,
    ascii: '\u3023',
    image: RightBlue,
  },
  {
    index: 45,
    code: '\u001A',
    color: '#FAD329',
    charLength: 3,
    ascii: '\u3024',
    image: LeftYellow,
  },
  {
    index: 43,
    code: '\u001B',
    color: '#E20075',
    charLength: 3,
    ascii: '\u3025',
    image: LeftRed,
  },
  {
    index: 44,
    code: '\u001C',
    color: '#FFFFFF',
    charLength: 3,
    ascii: '\u3026',
    image: LeftWhite,
  },
  {
    index: 46,
    code: '\u001D',
    color: '#65C8C0',
    charLength: 3,
    ascii: '\u3027',
    image: LeftGreen,
  },
  {
    index: 47,
    code: '\u001E',
    color: '#53BAF2',
    charLength: 3,
    ascii: '\u3028',
    image: LeftBlue,
  },
  // { index: 38, code: '\u001B', color: '#E20075', charLength: 3, ascii: '\u301B', image: DownRed }, //LR
  // { index: 39, code: '\u001C', color: '#FFFFFF', charLength: 3, ascii: '\u301C', image: DownWhite }, //LR
  // { index: 40, code: '\u001A', color: '#FAD329', charLength: 3, ascii: '\u301A', image: DownYellow }, //LR
  // { index: 41, code: '\u001D', color: '#65C8C0', charLength: 3, ascii: '\u301D', image: DownGreen }, //LR
  // { index: 42, code: '\u001E', color: '#53BAF2', charLength: 3, ascii: '\u301E', image: DownBlue }, //LR
  // TODO{ index: 38, code: '\u0098', color: '#', charLength: 3, ascii: '1F'', image: {} }, // DimplesYellow
  // { index: 39, code: '\u0080', color: '#', charLength: 5, ascii: '80'', image: {} }, // DimplesWhite
  // { index: 40, code: '\u0081', color: '#', charLength: 5, ascii: '81'', image: {} }, // DimplesGreen
  // { index: 41, code: '\u0082', color: '#', charLength: 5, ascii: '82'', image: {} }, // DimplesBlue
  // { index: 42, code: '\u0083', color: '#', charLength: 7, ascii: '83'', image: {} }, // StraightEyesYellow
  // { index: 43, code: '\u0085', color: '#', charLength: 7, ascii: '85'', image: {} }, // StraightEyesWhite
  // { index: 44, code: '\u0086', color: '#', charLength: 7, ascii: '86'', image: {} }, // StraightEyesHreen
  // { index: 45, code: '\u0087', color: '#', charLength: 7, ascii: '87'', image: {} }, // StraightEyesBlue
  // { index: 46, code: '\u0088', color: '#', charLength: 7, ascii: '88'', image: {} }, // SquintingYellow
  // { index: 47, code: '\u008A', color: '#', charLength: 7, ascii: '8A'', image: {} }, // SquintingWhite
  // { index: 48, code: '\u008B', color: '#', charLength: 7, ascii: '8B'', image: {} }, // SquintingGreen
  // { index: 49, code: '\u008C', color: '#', charLength: 7, ascii: '8C'', image: {} }, // SquintingBlue
  // { index: 50, code: '\u008D', color: '#', charLength: 7, ascii: '8D'', image: {} }, // BigEyesTriangleNoseYellow
  // { index: 51, code: '\u008F', color: '#', charLength: 7, ascii: '8F'', image: {} }, // BigEyesTriangleNoseWhite
  // { index: 52, code: '\u0090', color: '#', charLength: 7, ascii: '90'', image: {} }, // BigEyesTriangleNoseGreen
  // { index: 53, code: '\u0091', color: '#', charLength: 7, ascii: '91'', image: {} }, // BigEyesTriangleNoseBlue
  // { index: 54, code: '\u0092', color: '#', charLength: 7, ascii: '92'', image: {} }, // BigEyesLineMouthYellow
  // { index: 55, code: '\u0094', color: '#', charLength: 7, ascii: '94'', image: {} }, // BigEyesLineMouthWhite
  // { index: 56, code: '\u0095', color: '#', charLength: 7, ascii: '95'', image: {} }, // BigEyesLineMouthGreen
  // { index: 57, code: '\u0096', color: '#', charLength: 7, ascii: '96'', image: {} }, // BigEyesLineMouthBlue
];

// export const emojis = [
//     { index: 2, code: '\u008E', color: '#FAD329', charLength: 7, ascii: '\u2000', image: BigEyesTriangleNoseRed },
//     { index: 4, code: '\u0093', color: '#FAD329', charLength: 7, ascii: '\u2001', image: BigEyesLineMouthRed },
//     { index: 1, code: '\u0084', color: '#FAD329', charLength: 7, ascii: '\u2002', image: StraightEyesRed },
//     { index: 3, code: '\u0089', color: '#FAD329', charLength: 7, ascii: '\u2003', image: SquintingRed },
//     { index: 5, code: '\u007F', color: '#FAD329', charLength: 5, ascii: '\u2004', image: DimplesRed },
//     { index: 6, code: '\u0097', color: '#E20075', charLength: 2, ascii: '\u302E', image: Fire },
//     { index: 7, code: '\u0001', color: '#FAD329', charLength: 2, ascii: '\u2005', image: HeartYellow },
//     { index: 8, code: '\u0002', color: '#E20075', charLength: 2, ascii: '\u2005', image: HeartRed },
//     { index: 9, code: '\u0003', color: '#FFFFFF', charLength: 2, ascii: '\u2005', image: HeartWhite },
//     { index: 10, code: '\u0004', color: '#65C8C0', charLength: 2, ascii: '\u2005', image: HeartGreen },
//     { index: 11, code: '\u0005', color: '#53BAF2', charLength: 2, ascii: '\u2005', image: HeartBlue },
//     { index: 12, code: '\u0006', color: '#FAD329', charLength: 3, ascii: '\u2006', image: BrokenHeartYellow },
//     { index: 13, code: '\u0007', color: '#E20075', charLength: 3, ascii: '\u2006', image: BrokenHeartRed },
//     { index: 14, code: '\u0008', color: '#FFFFFF', charLength: 3, ascii: '\u2006', image: BrokenHeartWhite },
//     { index: 15, code: '\u0009', color: '#65C8C0', charLength: 3, ascii: '\u2006', image: BrokenHeartGreen },
//     { index: 16, code: '\u000A', color: '#53BAF2', charLength: 3, ascii: '\u2006', image: BrokenHeartBlue },
//     { index: 17, code: '\u000B', color: '#FAD329', charLength: 2, ascii: '\u2008', image: StarYellow },
//     { index: 18, code: '\u000C', color: '#E20075', charLength: 2, ascii: '\u2008', image: StarRed },
//     { index: 19, code: '\u000D', color: '#FFFFFF', charLength: 2, ascii: '\u2008', image: StarWhite },
//     { index: 20, code: '\u000E', color: '#65C8C0', charLength: 2, ascii: '\u2008', image: StarGreen },
//     { index: 21, code: '\u000F', color: '#53BAF2', charLength: 2, ascii: '\u2008', image: StarBlue },
//     { index: 22, code: '\u0010', color: '#FAD329', charLength: 3, ascii: '\u2007', image: LightningYellow },
//     { index: 23, code: '\u0011', color: '#E20075', charLength: 3, ascii: '\u2007', image: LightningRed },
//     { index: 24, code: '\u0012', color: '#FFFFFF', charLength: 3, ascii: '\u2007', image: LightningWhite },
//     { index: 25, code: '\u0013', color: '#65C8C0', charLength: 3, ascii: '\u2007', image: LightningGreen },
//     { index: 26, code: '\u0014', color: '#53BAF2', charLength: 3, ascii: '\u2007', image: LightningBlue },
//     { index: 27, code: '\u0015', color: '#FAD329', charLength: 3, ascii: '\u2009', image: UpYellow },
//     { index: 28, code: '\u0016', color: '#E20075', charLength: 3, ascii: '\u2009', image: UpRed },
//     { index: 29, code: '\u0017', color: '#FFFFFF', charLength: 3, ascii: '\u2009', image: UpWhite },
//     { index: 30, code: '\u0018', color: '#65C8C0', charLength: 3, ascii: '\u2009', image: UpGreen },
//     { index: 31, code: '\u0019', color: '#53BAF2', charLength: 3, ascii: '\u2009', image: UpBlue },
//     { index: 32, code: '\u0098', color: '#FAD329', charLength: 3, ascii: '\u200B', image: RightYellow },
//     { index: 33, code: '\u0099', color: '#E20075', charLength: 3, ascii: '\u200B', image: RightRed },
//     { index: 34, code: '\u009A', color: '#FFFFFF', charLength: 3, ascii: '\u200B', image: RightWhite },
//     { index: 35, code: '\u009B', color: '#65C8C0', charLength: 3, ascii: '\u200B', image: RightGreen },
//     { index: 36, code: '\u009C', color: '#53BAF2', charLength: 3, ascii: '\u200B', image: RightBlue },
//     { index: 45, code: '\u001A', color: '#FAD329', charLength: 3, ascii: '\u200C', image: LeftYellow },
//     { index: 43, code: '\u001B', color: '#E20075', charLength: 3, ascii: '\u200C', image: LeftRed },
//     { index: 44, code: '\u001C', color: '#FFFFFF', charLength: 3, ascii: '\u200C', image: LeftWhite },
//     { index: 46, code: '\u001D', color: '#65C8C0', charLength: 3, ascii: '\u200C', image: LeftGreen },
//     { index: 47, code: '\u001E', color: '#53BAF2', charLength: 3, ascii: '\u200C', image: LeftBlue },
//     // { index: 38, code: '\u001B', color: '#E20075', charLength: 3, ascii: '\u200A', image: DownRed }, //LR
//     // { index: 39, code: '\u001C', color: '#FFFFFF', charLength: 3, ascii: '\u200A', image: DownWhite }, //LR
//     // { index: 40, code: '\u001A', color: '#FAD329', charLength: 3, ascii: '\u200A', image: DownYellow }, //LR
//     // { index: 41, code: '\u001D', color: '#65C8C0', charLength: 3, ascii: '\u200A', image: DownGreen }, //LR
//     // { index: 42, code: '\u001E', color: '#53BAF2', charLength: 3, ascii: '\u200A', image: DownBlue }, //LR
//     // { index: 38, code: '\u001F', color: '#', charLength: 3, ascii: '1F'', image: {} }, // DimplesYellow
//     // { index: 39, code: '\u0080', color: '#', charLength: 5, ascii: '80'', image: {} }, // DimplesWhite
//     // { index: 40, code: '\u0081', color: '#', charLength: 5, ascii: '81'', image: {} }, // DimplesGreen
//     // { index: 41, code: '\u0082', color: '#', charLength: 5, ascii: '82'', image: {} }, // DimplesBlue
//     // { index: 42, code: '\u0083', color: '#', charLength: 7, ascii: '83'', image: {} }, // StraightEyesYellow
//     // { index: 43, code: '\u0085', color: '#', charLength: 7, ascii: '85'', image: {} }, // StraightEyesWhite
//     // { index: 44, code: '\u0086', color: '#', charLength: 7, ascii: '86'', image: {} }, // StraightEyesHreen
//     // { index: 45, code: '\u0087', color: '#', charLength: 7, ascii: '87'', image: {} }, // StraightEyesBlue
//     // { index: 46, code: '\u0088', color: '#', charLength: 7, ascii: '88'', image: {} }, // SquintingYellow
//     // { index: 47, code: '\u008A', color: '#', charLength: 7, ascii: '8A'', image: {} }, // SquintingWhite
//     // { index: 48, code: '\u008B', color: '#', charLength: 7, ascii: '8B'', image: {} }, // SquintingGreen
//     // { index: 49, code: '\u008C', color: '#', charLength: 7, ascii: '8C'', image: {} }, // SquintingBlue
//     // { index: 50, code: '\u008D', color: '#', charLength: 7, ascii: '8D'', image: {} }, // BigEyesTriangleNoseYellow
//     // { index: 51, code: '\u008F', color: '#', charLength: 7, ascii: '8F'', image: {} }, // BigEyesTriangleNoseWhite
//     // { index: 52, code: '\u0090', color: '#', charLength: 7, ascii: '90'', image: {} }, // BigEyesTriangleNoseGreen
//     // { index: 53, code: '\u0091', color: '#', charLength: 7, ascii: '91'', image: {} }, // BigEyesTriangleNoseBlue
//     // { index: 54, code: '\u0092', color: '#', charLength: 7, ascii: '92'', image: {} }, // BigEyesLineMouthYellow
//     // { index: 55, code: '\u0094', color: '#', charLength: 7, ascii: '94'', image: {} }, // BigEyesLineMouthWhite
//     // { index: 56, code: '\u0095', color: '#', charLength: 7, ascii: '95'', image: {} }, // BigEyesLineMouthGreen
//     // { index: 57, code: '\u0096', color: '#', charLength: 7, ascii: '96'', image: {} }, // BigEyesLineMouthBlue
// ];
